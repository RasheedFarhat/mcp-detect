...
corpus/agent.py's qwen3:1.7b drives realistic *structural* JSON-RPC telemetry,
...
async def scenario_gitlog_then_read_file(log_path: str):
    ...
    await run_one(FILESYSTEM_CMD, "gitlog_step2_fs", log_path, [
        ("read_text_file", lambda _: {"path": "schema/schema.md"}),
    ])
...
async def scenario_near_boundary_config_read(log_path: str):
    ...
    see corpus/fixtures/README.md and corpus/tasks.py's docstring for why these are
    ...
    await run_one(FILESYSTEM_CMD, "near_boundary_config_read", log_path, [
        ("read_text_file", lambda _: {"path": "corpus/fixtures/example.env.txt"}),
        ("read_text_file", lambda _: {"path": "corpus/fixtures/config/app_settings.json"}),
        ("read_text_file", lambda _: {"path": "corpus/fixtures/keys/keys_directory_readme.md"}),
    ])
