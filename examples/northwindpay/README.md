# examples/northwindpay/ — synthetic ground-truth environment for assessor validation
...
this corpus specifically -- not copied from `lab/attacks/servers/*.py`,
...
`lab/attacks/harness.py`'s env-var-parametrized variant factory: `generate_corpus.py`
...
python3 framework/audit_report.py examples/northwindpay/telemetry.jsonl \
    --known-good examples/northwindpay/known_good_bom.json --markdown > examples/northwindpay/audit_report_run.md
...
[`SYNTHETIC-READINESS-REPORT.md`](SYNTHETIC-READINESS-REPORT.md) is the
polished report built from this validated run: an executive summary,
severity-ranked findings with remediation, and an explicit "what this
assessment cannot see" section. The remaining files in this directory are
the evidence and validation harness behind that report.
