...
Scores `framework/audit_report.py` against `examples/northwindpay/GROUND_TRUTH.json` --
...
only `examples/northwindpay/score_against_ground_truth.py`, invoked after the fact,
does.
...
`examples/northwindpay/audit_report_run.md` / `.json` are the actual, unedited output
...
`lab/analysis/report.py`'s `load_inputs()` / `evasion_report.py`'s frozen
...
`examples/northwindpay/telemetry.jsonl` (see the raw counts below) confirms the fix
...
**Root cause**: `lab/baseline/watch.py`'s derived drift record already carries a
...
changes to `lab/baseline/watch.py` or to `wazuh/local_rules.xml`'s `100201` --
