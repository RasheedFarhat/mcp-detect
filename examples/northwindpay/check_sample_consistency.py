...
(examples/northwindpay/telemetry.jsonl). This project's whole brand is "never claim a
number you can't reproduce" -- its own showcase deliverable drifting from its
own source data (the report once said 4,032/536 while the seeded corpus
...
SYNTHETIC-READINESS-REPORT.md and audit_report_run.md match it. Loud,
specific failure -- never a silent pass. Stdlib-only, no live stack needed.

Usage: python3 examples/northwindpay/check_sample_consistency.py   (exit 0 / 1)
...
TELEMETRY = REPO_ROOT / "examples" / "northwindpay" / "telemetry.jsonl"
SAMPLE_REPORT = REPO_ROOT / "examples" / "northwindpay" / "SYNTHETIC-READINESS-REPORT.md"
RAW_RUN = REPO_ROOT / "examples" / "northwindpay" / "audit_report_run.md"
...
def check_sample_report(gt: dict) -> list[str]:
    """Synthetic report line: 'N MCP servers, M agent sessions, R telemetry records'."""
...
def main() -> int:
    ...
    if errors:
        ...
        for e in errors:
            print(f"  - {e}")
        print("Regenerate the artifact against the current examples/northwindpay/telemetry.jsonl "
              "(see examples/northwindpay/ASSESSMENT-VALIDATION.md's run recipe).")
        return 1
