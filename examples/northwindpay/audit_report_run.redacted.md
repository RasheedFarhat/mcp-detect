...
**Generated**: 2026-07-12T08:10:46.038Z (`mcp-detect-audit-report/1.0`)
**Client telemetry**: `examples/northwindpay/telemetry.redacted.jsonl` (sha256 `55376b64c83d71f9...`, 4046 records)
...
- Tool-name spoofing evades this rule -- CONFIRMED via real wazuh-logtest (attack_evasion_e1_toolname_spoof, lab/attacks/path_traversal_evasion_harness.py): a tool named 'search_files' performing an actual traversal read (path=../../../etc/passwd) is negate-excluded and produces no alert (final match: 100100). Same class E5 already proved for 100103-100107's own negate-list (docs/PHASE5-REPORT.md). Not fixed this round -- the negate-list is what closed a real, measured 20/4727 benign FP (search_files' legitimate navigation), and narrowing it further would reopen that FP.
