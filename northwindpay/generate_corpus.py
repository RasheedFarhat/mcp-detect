...
schema v1 telemetry, built to the same shape corpus/handauthored.py and
attacks/harness.py produce, but by a DIFFERENT method than either: no live
docker/MCP process is spawned here (unlike both of those), and no reuse of
attacks/harness.py's env-var-parametrized variant factory (POISON_TOOL_NAME/
...
records with the exact field semantics proxy/proxy.py's own build_record()/
...
matters for correctness) is imported unmodified from proxy/hashing.py, never
reimplemented.
...
fictional business context -- not copied from attacks/servers/*.py or
data/evasion_corpus_v1.jsonl.

Output (all committed, public synthetic research data):
  northwindpay/telemetry.jsonl        -- the full corpus, chronologically sorted
  northwindpay/known_good_bom.json    -- client-declared known-good server list
  northwindpay/GROUND_TRUTH.json      -- sealed scoring manifest (assessment
                                          run must never read this)

Usage: python3 northwindpay/generate_corpus.py
"""
...
# ---------------------------------------------------------------------------
# Minimal record assembly -- mirrors proxy/proxy.py's build_record()/observe()
...
def build_fs_workspace() -> None:
    ...
    # redaction/redact.py's minimization -- not its six-shape recognition --
    ...
    # 100101/100108, redaction/DESIGN.md) -- a disclosed, not hidden,
...
def build_slack() -> None:
    ...
    cmd = "python3 /opt/northwindpay/mcp-servers/slack-connector/server.py"
...
def build_expense_ocr() -> None:
    ...
    cmd = "python3 /opt/northwindpay/mcp-servers/expense-ocr/server.py"
...
def build_fx_rates() -> None:
    ...
    cmd = "python3 /opt/northwindpay/mcp-servers/fx-rates/server.py"
    ...
    # CLASS B DECOY #2: legitimate server version bump. Same tool schema/
...
def build_production_ledger() -> None:
    ...
    # from attacks/servers/traversal_read_server.py or the frozen corpora.
...
def build_support_ticket_bot() -> None:
    ...
    cmd = "python3 /opt/northwindpay/mcp-servers/support-ticket-bot/server.py"
    ...
    # or attacks/servers/exfil_sink_server.py.
...
def build_shadow_crm() -> None:
    ...
    cmd = "python3 /opt/northwindpay/shadow-tools/mcp-crm-lite/server.py"
...
def main() -> None:
    ...
    ground_truth = {
        ...
        "known_good_bom_path": "northwindpay/known_good_bom.json",
        "telemetry_path": "northwindpay/telemetry.jsonl",
