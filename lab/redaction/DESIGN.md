...
An earlier version of this document and `lab/redaction/redact.py` described a
...
in any recoverable form.** See `lab/redaction/REDACTION-VALIDATION.md` for
...
recognized in a new place (lab/redaction/DESIGN.md's own prior version didn't
...
`framework/audit_report.py`, `lab/baseline/watch.py`, and `wazuh/
...
| `tool_description_hash`, `server_version_hash` | AI-BOM inventory, rug-pull baseline drift | `abom.py`, `lab/baseline/watch.py` |
...
| `raw.result.tools` | THE detection signal for 100102 (tool poisoning) + exact input to `tool_description_hash` | `wazuh/local_rules.xml`, `lab/proxy/hashing.py`'s recipe |
| `raw.result.serverInfo` | exact input to `server_version_hash` | `lab/proxy/hashing.py`'s recipe |
...
- **`tool_arguments.path`** can itself reveal a username or internal
  ...
  which IS the path string — see `lab/redaction/redact.py --report`'s
...
## Residual-disclosure report (`lab/redaction/redact.py --report`)
