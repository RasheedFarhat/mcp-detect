...
Deliberately NOT built on lab/attacks/harness.py (kept frozen since Phase 4 for
...
following the exact precedent lab/attacks/evasion_harness.py already set for
...
telemetry.jsonl directly -- the same file lab/attacks/harness.py's own
...
root -- the same server lab/attacks/harness.py's credential_exfil attack
already uses -- so the traversal argument is sent through the real proxy
and the real MCP lab/client/server stack. Whether the underlying server
...
Usage: python3 lab/attacks/path_traversal_harness.py --log-path <path>
