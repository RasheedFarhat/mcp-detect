...
identical schema-v1 output). Modeled on lab/corpus/handauthored.py's
...
Usage: python3 lab/attacks/harness.py --log-path <path> [--sink-url URL]
The exfil sink (lab/attacks/sink.py) must already be running before this script
...
async def attack_rug_pull_drift_shapes(log_path: str) -> None:
    ...
    lab/baseline/watch.py's corpus-replay proof needs description-only and
