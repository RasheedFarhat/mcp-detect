...
Deliberately reuses analysis/report.py's primitives (rule-sync gate, batch
wazuh-logtest runner, join/normalization) rather than reimplementing them --
see docs/PHASE5-DESIGN.md Section 2. Also reuses baseline/watch.py directly
for the rug-pull evasions (E10-E12), since "did this drift" is a two-stage
question here: does baseline/watch.py emit a drift record at all, and if so,
does 100201 fire on it -- both via the real code, never asserted.

Usage:
  python3 analysis/evasion_report.py     generate docs/PHASE5-REPORT.md
"""
import sys
...
def render_report(*, rule_sha, wazuh_version, n_benign, benign_alerts,
    ...
    out = []
    out.append("<!-- GENERATED FILE -- produced by analysis/evasion_report.py. "
                "Do not hand-edit; re-run `python3 analysis/evasion_report.py` to update. -->")
    ...
    out.append("Two-stage measurement: does `baseline/watch.py` emit a drift record at all "
                "(the detector's own decision), and if so, does `100201` fire on it (Wazuh's "
                "side). Both checked via the real code, not asserted.\n")
    out.append("| class | baseline/watch.py emitted drift? | 100201 fired? |")
    out.append("|---|---|---|")
    ...
    if unexpected_drift:
        ...
        out.append("`baseline/watch.py` doesn't know or care what an evasion class is testing — "
        ...
        out.append("\n**This isn't just an observation that happens to hold — it's structurally "
                    ...
                    "`baseline/watch.py` does downstream. The 5 firings in the table above live "
    ...
    out.append("\nThe only genuine generalization available for E6 (the 6th-argument-key "
                ...
                "name, architecturally similar to how `baseline/watch.py` already moved "
