...
Design rationale: redaction/DESIGN.md (read that first -- it also documents
...
authored free text. `redaction/redact.py --report` (see `build_residual_
...
is `proxy/proxy.py`, unmodified and reused as-is (`engagement/RUNBOOK.md`).
...
Usage:
  python3 redaction/redact.py <input.jsonl> <output.jsonl>
  python3 redaction/redact.py <input.jsonl> <output.jsonl> --report
"""
...
# baseline/watch.py, and wazuh/local_rules.xml's dynamic <field name="...">
# references actually read, re-derived directly from those five files (not
# guessed). Everything else is minimized. See redaction/DESIGN.md's
...
def redact_record(record: dict) -> dict:
    ...
    minimization per redaction/DESIGN.md, not blocklist substitution."""
    record = dict(record)
    ...
    # Recomputed from the now-minimized raw, using proxy/proxy.py's own
...
# open-ended problem (see redaction/DESIGN.md) -- it is NOT a guarantee that
