...
Proves `redaction/redact.py`'s corrected design (`redaction/DESIGN.md` --
data minimization by allowlist, not blocklist redaction) against
`northwindpay/telemetry.jsonl`, the same ground-truth-labeled corpus
`northwindpay/ASSESSMENT-VALIDATION.md` scored the un-minimized assessor
...
python3 redaction/redact.py northwindpay/telemetry.jsonl northwindpay/telemetry.redacted.jsonl --report
.venv/bin/python3 schema/validate.py northwindpay/telemetry.redacted.jsonl

python3 framework/audit_report.py northwindpay/telemetry.redacted.jsonl \
    --known-good northwindpay/known_good_bom.json --json > northwindpay/audit_report_run.redacted.json
python3 framework/audit_report.py northwindpay/telemetry.redacted.jsonl \
    --known-good northwindpay/known_good_bom.json --markdown > northwindpay/audit_report_run.redacted.md
python3 northwindpay/score_against_ground_truth.py northwindpay/audit_report_run.redacted.json

.venv/bin/python3 framework/tests/test_redaction_secret_survival.py
```

`northwindpay/telemetry.redacted.jsonl` and `northwindpay/
audit_report_run.redacted.{md,json}` are committed, unedited outputs.

## Schema validity

`schema/validate.py northwindpay/telemetry.redacted.jsonl`: **4046 records
valid, 0 invalid**.
...
contain, read via an ordinary `read_text_file` call, `northwindpay/
...
oversight; it's exactly the disclosed residual `redaction/DESIGN.md` names,
and exactly what `redaction/redact.py --report`'s residual-disclosure pass
...
marker (`redaction/redact.py`'s `_private_key_marker()`). This corpus never
...
Per `redaction/DESIGN.md`: `tool_arguments.path` and `raw.result.tools`/
...
both are named in `redaction/DESIGN.md`'s own "What this design does NOT
claim" section.
